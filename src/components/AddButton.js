...
const AddButton = () =>{
    ...
    const onOfficerAddClick= (ev) => {
        ev.preventDefault()
        const data = {policeOfficerData:{PO_Name: pname, Badge_num: bnum, Precinct_ID: pid, Phone: pnumber, Status: status}}
        axios.post('http://localhost:3005/main_page/add_officer', data).then(res=>console.log(res.data));
    ...
    const onCriminalAddClick= (ev) => {
        ev.preventDefault()
        const data = {criminalIdData:{Name: cname, Criminal_ID: cid, Phone_num: cnumber, Address: address}, criminalData:{Criminal_ID: cid, Violent_Offender_Status: offenderstatus, Probation_Status: probationstatus, Aliases: Aliases}}
        axios.post('http://localhost:3005/main_page/add_criminal', data).then(res=>console.log(res.data));

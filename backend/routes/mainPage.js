...
router.route("/add_criminal").post((req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    const idTableSql =
      "INSERT INTO Criminal_ID_Table (Criminal_ID, Phone_num, Name, Address) VALUES (?, ?, ?, ?)";
    const idTableValues = [
      data.Criminal_ID,
      data.Phone_num,
      data.Name,
    ...
    db.query(idTableSql, idTableValues, (idTableErr) => {
      if (idTableErr) {
        return db.rollback(() => {
          console.error(
            "Error adding data to Criminal_ID_Table:",
            idTableErr.message
          );
          res.status(500).json({ error: "Internal Server Error" });
        });
      ...
      const criminalSql =
        "INSERT INTO Criminal (Criminal_ID ,Violent_Offender_Status, Probation_Status, Aliases) VALUES (?, ?, ?, ?)";
      const criminalValues = [
        data.Criminal_ID,
        data.Violent_Offender_Status,
...
router.route("/add_officer").post((req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    const policeOfficerSql =
      "INSERT INTO Police_Officer (PO_Name, Badge_num, Precinct_ID, Phone, Status) VALUES (?, ?, ?, ?, ?)";
    ...
    const procedureValues = [data.Precinct_ID];

    db.query(
      policeOfficerSql,
      policeOfficerValues,
      (policeOfficerErr, policeOfficerResults) => {
        if (policeOfficerErr) {
          return db.rollback(() => {
            console.error(
              "Error adding police officer:",
              policeOfficerErr.message
            );
            res.status(500).json({ error: "Internal Server Error" });
          });
        }
        ...
        db.query(
          ...
          (policeOfficerErr, policeOfficerResults) => {
            ...
            db.commit((commitErr) => {
              ...
              res
                .status(201)
                .json({ message: "Police officer added successfully" });
            });
          }
        );
      }
...
router.route("/deletePoliceOfficer/:badgeNum").delete((req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    const deletePoliceOfficerSql =
      "DELETE FROM Police_Officer WHERE Badge_num = ?";

    const deleteArrestingOfficerSql =
      "DELETE FROM Arresting_Officers WHERE Badge_Num = ?";

    const selectSql =
      "SELECT Precinct_ID FROM Police_Officer WHERE Badge_num LIKE ?;";
    ...
    db.query(selectSql, [badgeNum], (selectErr, selectResults) => {
      ...
      db.query(
        deleteArrestingOfficerSql,
        [badgeNum],
        (deleteArrestingOfficerErr, deleteArrestingOfficerResults) => {
          if (deleteArrestingOfficerErr) {
            return db.rollback(() => {
              console.error(
                "Error deleting police officer:",
                deleteArrestingOfficerErr.message
              );
              res.status(500).json({ error: "Internal Server Error" });
            });
          }
          ...
          db.query(
            deletePoliceOfficerSql,
            [badgeNum],
            (deletePoliceOfficerErr, deletePoliceOfficerResults) => {
              if (deletePoliceOfficerErr) {
                return db.rollback(() => {
                  console.error(
                    "Error deleting police officer:",
                    deletePoliceOfficerErr.message
                  );
                  res.status(500).json({ error: "Internal Server Error" });
                });
              }
              ...
              db.query(
                ...
                (policeOfficerErr, policeOfficerResults) => {
                  ...
                  db.commit((commitErr) => {
                    ...
                    res
                      .status(200)
                      .json({ message: "Police officer deleted successfully" });
                  });
                }
              );
            }
...
router.route("/deleteCriminal/:criminalId").delete((req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    db.query(
      deleteSentencingSql,
      [criminalId],
      (deleteSentencingErr, deleteSentencingResults) => {
        if (deleteSentencingErr) {
          return db.rollback(() => {
            console.error(
              "Error deleting criminal:",
              deleteSentencingErr.message
            );
            res.status(500).json({ error: "Internal Server Error" });
          });
        }
        ...
        db.query(
          deleteCauseSql,
          [criminalId],
          (deleteCauseErr, deleteCauseResults) => {
            if (deleteCauseErr) {
              return db.rollback(() => {
                console.error(
                  "Error deleting criminal:",
                  deleteCauseErr.message
                );
                res.status(500).json({ error: "Internal Server Error" });
              });
            }
            // Delete from Criminal table
            const deleteCriminalSql =
              "DELETE FROM Criminal WHERE Criminal_ID = ?";

            db.query(
              deleteCriminalSql,
              [criminalId],
              (deleteCriminalErr, deleteCriminalResults) => {
                if (deleteCriminalErr) {
                  return db.rollback(() => {
                    console.error(
                      "Error deleting criminal:",
                      deleteCriminalErr.message
                    );
                    res.status(500).json({ error: "Internal Server Error" });
                  });
                }
                ...
                db.commit((commitErr) => {
                  ...
                  res
                    .status(200)
                    .json({ message: "Criminal deleted successfully" });
                });
              }
            );
          }
...
router.route("/update_police_officer/:badgeNum").put((req, res) => {
  ...
  if (
    !updatedPoliceOfficerData ||
    Object.keys(updatedPoliceOfficerData).length === 0
  ) {
  ...
  db.beginTransaction((err) => {
    ...
    const updatePoliceOfficerValues = updateFields.map(
      (field) => updatedPoliceOfficerData[field]
    );
    updatePoliceOfficerValues.push(badgeNum);

    db.query(
      updatePoliceOfficerSql,
      updatePoliceOfficerValues,
      (updateErr, updateResults) => {
        ...
        db.commit((commitErr) => {
          ...
          res
            .status(200)
            .json({ message: "Police officer updated successfully" });
        });
      }
    );
  });
...
router.route("/update_criminal/:criminalId").put((req, res) => {
  ...
  if (
    (!updatedCriminalData || Object.keys(updatedCriminalData).length === 0) &&
  ...
  db.beginTransaction((err) => {
    ...
    const updateCriminalValues = updateCriminalFields
      .map((field) => `${field} = ?`)
      .join(", ");
    ...
    const updateCriminalIdValues = updateCriminalIdFields
      .map((field) => `${field} = ?`)
      .join(", ");
    ...
    const updateCriminalSqlValues = updateCriminalFields.map(
      (field) => updatedCriminalData[field]
    );
    updateCriminalSqlValues.push(criminalId);

    const updateCriminalIdSqlValues = updateCriminalIdFields.map(
      (field) => updatedCriminalIdData[field]
    );
    ...
    db.query(
      updateCriminalSql,
      updateCriminalSqlValues,
      (updateCriminalErr, updateCriminalResults) => {
        if (updateCriminalErr) {
          return db.rollback(() => {
            console.error(
              "Error updating criminal:",
              updateCriminalErr.message
            );
            res.status(500).json({ error: "Internal Server Error" });
          });
        }
        ...
        db.query(
          updateCriminalIdSql,
          updateCriminalIdSqlValues,
          (updateCriminalIdErr, updateCriminalIdResults) => {
            if (updateCriminalIdErr) {
              return db.rollback(() => {
                console.error(
                  "Error updating criminal ID:",
                  updateCriminalIdErr.message
                );
                res.status(500).json({ error: "Internal Server Error" });
              });
            }
            ...
            db.commit((commitErr) => {
              ...
              res
                .status(200)
                .json({
                  message: "Criminal and Criminal ID updated successfully",
                });
            });
          }
        );
...
router.route("/searchCriminal/:name").post((req, res) => {
  ...
  const db = icreateConnection(req.session.userId);
  const searchQuery =
    "SELECT t.Phone_num, t.Name, t.Address, c.Criminal_ID, c.Violent_Offender_Status, c.Probation_Status, c.Aliases FROM Criminal_ID_Table t INNER JOIN Criminal c ON c.Criminal_ID = t.Criminal_ID WHERE t.Name LIKE ?";
...
// API endpoint to add data to a specified table
router.route("/add_data").post((req, res) => {
  const db = icreateConnection(req.session.userId);
  ...
  db.beginTransaction((err) => {
    ...
    if (!data) {
      console.error("Invalid Input");
      res.status(500).json({ error: "Cannot add empty data" });
      return;
    }

    // Define the table-specific columns and values
    const tableColumns = Object.keys(data).join(", ");
    const tableValues = Object.values(data);

    // Construct SQL query
    const sql = `INSERT INTO ${tableName} (${tableColumns}) VALUES (${Array(
      tableValues.length
    )
      .fill("?")
      .join(", ")})`;
    ...
    // Execute the query
    db.query(sql, tableValues, (queryErr) => {
      if (queryErr) {
        return db.rollback(() => {
          console.error(`Error adding data to ${tableName}:`, queryErr.message);
          res.status(500).json({ error: "Internal Server Error" });
        });
      }
